...
var (
	clusterTypeIsSupported = NamedCheck("Cluster type is supported", func() error {
		...
		case ClusterTypeKind:
			return fmt.Errorf("We don't currently support Kind clusters. To create a test cluster to try out Kubestar, use minikube instead.  ")
		case ClusterTypeDockerDesktop:
			return fmt.Errorf("Docker for desktop is not supported. To create a test cluster to try out Kubestar, use minikube instead.  ")
	...
	allowListClusterCheck = NamedCheck("Cluster type is in list of known supported types", func() error {
		...
		return errors.New("Cluster type is not in list of known supported cluster types.")
	})
)
